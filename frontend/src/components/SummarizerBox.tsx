  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (activeTab === "text") {
        ...
        resText = await summarizeApi.summarizeFile(file);
      }
      setResult(resText);
    } catch (err: any) {
      ...
      <div className="flex border-b border-border">
        <button
          ...
          className={`flex-1 py-4 flex items-center justify-center gap-2 font-medium transition-colors ${
            activeTab === "text" ? "text-primary border-b-2 border-primary bg-primary/5" : "text-muted-foreground hover:bg-muted/50"
          }`}
        ...
        <button
          ...
          className={`flex-1 py-4 flex items-center justify-center gap-2 font-medium transition-colors ${
            activeTab === "file" ? "text-primary border-b-2 border-primary bg-primary/5" : "text-muted-foreground hover:bg-muted/50"
          }`}
      ...
      <div className="p-6 md:p-8 flex flex-col gap-6">
          ...
          {activeTab === "text" ? (
            ...
            >
              ...
              >
                ...
                >
                  {file ? <File className="w-8 h-8 text-primary" /> : <Upload className="w-8 h-8" />}
                </motion.div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-between items-center bg-background/50 p-2 rounded-xl border border-border/50">
          <div className="flex gap-1 w-full sm:w-auto p-1 bg-muted rounded-lg">
            {(["short", "detailed", "bullet_points"] as SummaryType[]).map((type) => (
              <button
                key={type}
              ...
              >
                {type === "short" ? "Short" : type === "detailed" ? "Detailed" : "Bullets"}
              </button>
            ))}
          </div>
          ...
          <Button
            ...
            disabled={isLoading}
            className="w-full sm:w-auto font-semibold px-8 shadow-[0_0_15px_rgba(var(--primary),0.3)] hover:shadow-[0_0_25px_rgba(var(--primary),0.5)] transition-all relative overflow-hidden group"
          >
                  ...
                  {summaryType === "bullet_points" ? (
                    <ul className="space-y-2">
                      {result.split('\n').map((line, i) => {
                        ...
                        return <li key={i} className="leading-relaxed">{cleanLine}</li>;
                      })}
                    </ul>
                  ) : (
